...
export const RADIUS = {
  /** "바로 여기" 수준까지 좁힐 수 있게 50m부터 */
  min: 50,
  max: 3000,
  /** min이 50이므로 100 간격이면 50·150·250처럼 어긋납니다 */
  step: 50,
  default: 1000,
} as const;
 ...
 *
 * **하한이 던지는 거리보다 크면 안 됩니다.** 예전 하한 120m로는 반경 50m로 던져도
 * 착지점 주변 120m를 뒤져서 기준점에서 170m 떨어진 가게가 당첨될 수 있었습니다.
 *
 * 하한은 실측으로 40m로 정했습니다. 강남역에서 반경 50m로 16번 던져 착지점에서
 * 가장 가까운 음식점까지의 거리를 재보니 22~48m(중앙값 37m)였습니다. 가게는 건물
 * 안에 있고 사이에 도로와 블록이 있어서 임의의 지점 20m 안에는 대개 아무것도 없습니다.
 *
 *   하한 20m → 적중 0/13   (반경 50m가 사실상 플레이 불가)
 *   하한 30m → 적중 3/13
 *   하한 40m → 적중 11/13  ← 채택
 *   하한 50m → 적중 13/13  (판정 반경이 던지는 거리와 같아져 느슨해짐)
 *
 * 1000m 이상에서는 예전과 동일합니다(300m / 500m).
 */
export const REVEAL = {
  ratio: 0.3,
  minM: 40,

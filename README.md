...
바운스마다 물이 한 겹씩 빠지고, 마지막 바운스에서 수면이 걷히며 **돌이 떨어진 자리에서
가장 가까운 한 곳**이 공개됩니다. 마음에 안 들면 "맘에 안 들면 여긴 어때요?"를 열어
근처 3곳 중에서 바꿀 수 있습니다.
...
- **참가자 목록** — 결정한 사람이 없어도 방에 누가 있는지 보입니다. 계정이 없으므로
  피드 폴링에 닉네임을 얹는 하트비트 방식이고, 25초 소식이 없으면 목록에서 빠집니다
- **붐업 👍 / 붐따 👎** — 올라온 결과에 투표. 같은 걸 다시 누르면 취소, 반대쪽을 누르면 이동
- **매장 이름을 누르면 카카오맵으로** — 링크는 `place.map.kakao.com` 등 카카오 호스트만
  통과시킵니다. **다른 참가자 화면에 렌더되는 링크라 검증하지 않으면 누구나 임의 주소를
  방에 심을 수 있습니다**
- **본인이 올린 결과는 내릴 수 있습니다** (목록의 ✕). 서버가 올린 브라우저를 확인하며,
  `ownerId`는 응답에서 제거하고 `mine` 불리언으로만 내려갑니다
- **방 잠그기** — 방을 만든 사람만. 6자리 코드는 32⁶ ≈ 10.7억 조합이고 조회에 IP 상한이
  있어 무작위 참가는 사실상 불가능하지만, 코드가 유출됐을 때 인원이 모인 뒤 새 참가를
  막을 수 있습니다. 코드는 기본으로 가려두고 탭해서 드러냅니다
...
가까운 음식점까지의 거리를 재보니 22~48m(중앙값 37m)였습니다. 가게는 건물 안에 있고
사이에 도로와 블록이 있어서 임의의 지점 20m 안에는 대개 아무것도 없습니다.

| 하한 | 적중 (반경 안 착지 13회 중) |
| --- | --- |
| 20m | 0/13 — 반경 50m가 플레이 불가 |
| 30m | 3/13 |
| **40m** | **11/13** |
| 50m | 13/13 (판정 반경이 던지는 거리와 같아져 느슨해짐) |

...
기준점에서 돌을 던지면 지도 위를 통통 튀며 날아가고, 튀는 동안 지도는 블러로 가려집니다.
마지막 바운스에서 블러가 걷히며 그 자리에서 가장 가까운 식당이 공개됩니다.
...
| 값 | 기본 | 의미 |
| --- | --- | --- |
| `angleSigmaDeg` | 10° | 작으면 조준 게임, 크면 그냥 룰렛 |
| `distanceSpan` | 0.85 | 최대 파워에서 반경의 1.2배 → 밖으로 나가 허탕 가능 |
| `damping` | 0.6 | 홉이 짧아지는 비율 |
| `blurStartPx` | 16 | 릴리즈 직후 블러 |
